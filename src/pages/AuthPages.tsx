...
import { Eye, EyeOff, Mail, Lock, User, ArrowLeft, Coins, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 pt-14 md:pt-16 pb-8">
      <div className="w-full max-w-md">
        <button onClick={() => setCurrentPage('landing')} className="flex items-center gap-2 text-muted-foreground hover:text-foreground mb-6 transition-colors text-sm">
          <ArrowLeft className="w-4 h-4" /> Back
        </button>

        <div className="relative overflow-hidden rounded-3xl p-6 md:p-8 border border-primary/20">
          ...
          <div className="relative z-10">
            <div className="text-center mb-8">
              ...
              <h1 className="font-rajdhani text-3xl font-black mb-1">Welcome Back</h1>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-muted/60 border border-border rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-primary/60 focus:bg-muted transition-all"
                  />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full bg-muted/60 border border-border rounded-xl pl-10 pr-12 py-3 text-sm focus:outline-none focus:border-primary/60 focus:bg-muted transition-all"
                  />
            ...
            <div className="mt-6 text-center text-sm text-muted-foreground">
              ...
              <button onClick={() => setCurrentPage('signup')} className="text-primary hover:underline font-semibold">
                Sign up free
              </button>
            </div>
          </div>
...
export const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError('');
    const { ok, error: err } = await signup(username, email, password);
    if (!ok) setError(err || 'Something went wrong. Please try again.');
    setLoading(false);
  };
        ...
        <div className="relative overflow-hidden rounded-3xl p-6 md:p-8 border border-primary/20">
          ...
          <div className="relative z-10">
            <div className="text-center mb-6">
              <div className="text-5xl mb-3">🏆</div>
              <h1 className="font-rajdhani text-3xl font-black mb-1">Create Account</h1>
              <p className="text-muted-foreground text-sm">Get 1,000 free coins on signup</p>
              <div className="inline-flex items-center gap-2 mt-2.5 bg-yellow-400/10 border border-yellow-400/30 rounded-xl px-4 py-2">
                <Coins className="w-4 h-4 text-yellow-400" />
                <span className="text-yellow-400 text-sm font-bold">+1,000 starter coins 🪙</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {[
                { label: 'Username', icon: <User className="w-4 h-4" />, value: username, setter: setUsername, type: 'text', placeholder: 'CricketFan123' },
                { label: 'Email', icon: <Mail className="w-4 h-4" />, value: email, setter: setEmail, type: 'email', placeholder: 'you@example.com' },
              ].map(({ label, icon, value, setter, type, placeholder }) => (
                <div key={label}>
